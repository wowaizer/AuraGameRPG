...
{
	// Source = causer of the effect, Target = target of the effect (owner of this atrribute set), 
	...
	if(Data.EvaluatedData.Attribute == GetIncomingDamageAttribute())
	{
		...
		if(LocaIncomingDamage > 0.f)
		{
			...
			else
			{
				...
				if(CombatInterface)
				{
					CombatInterface->HitReaction(IsHit);
					IsHit = false;
				}*/
			}
			
			const bool bBlock = UAuraAbilitySystemLibrary::IsBlockedHit(Props.EffectContextHandle);
			const bool bCritHit = UAuraAbilitySystemLibrary::IsCriticalHit(Props.EffectContextHandle);
